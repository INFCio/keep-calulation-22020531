...
const forget = d.createElement("div").setAttribute({ class: "container" });

// header
const header = d.createElement(
  "header"
);
const h1 = d.createElement("h1", "Password Reset");
header.append(h1);
...
form.append({ ...label.setChildren("User Email") }, email, submit);
      ...
      d.post(
        ...
        if (res.result) {
          if (res.messege === "success") {
            ...
            errDiv.setChildren("User Email not found!");
            error.changeAttribute("style", "display: flex");
            submit
              .setChildren("Reset Password")
              .removeAttribute("disabled", "style");
    ...
    if (res.result) {
      if (res.messege === "success") {
        if (!otp._rendered) {
          form.insert(
            3,
            { ...label.setChildren("New Password") },
            password,
            { ...label.setChildren("Confirm New Password") },
            conPassword,
            { ...label.setChildren("OTP") },
            otp,
            span
          );
